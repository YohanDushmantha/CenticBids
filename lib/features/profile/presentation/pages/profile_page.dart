...
class ProfilePage extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: SafeArea(
          child: appStates?.appUser != null ? _authenticated(context, appStates.appUser) : _notAuthenticated(context),
        )
    );
  }
          ...
          Card(
            child: Container(
              padding: EdgeInsets.symmetric(vertical: 24, horizontal: 8),
                child: Center(child: Text('You are not signed in', textAlign: TextAlign.center, style: Theme.of(context)
                    .textTheme.headline3.copyWith(color: styles.notAuthenticatedTextColor
                , fontWeight: FontWeight.w500),))),
          ),
          SizedBox(height: 24,),
          TextButton(onPressed: () => _onTapSignIn(context), child: Text(
            S.of(context).signupAlreadyHaveAnAccount,
            style: TextStyle(
                fontSize: 18
            ),
          )),
          TextButton(onPressed: () => _onTapSignUp(context), child: Text(
  ...
  Widget _authenticated(BuildContext context, AppUser appUser){
    return Container(
      child: Column(
        ...
        children: [
          _getDataCard(context, S.of(context).signUpPageFirstNameFieldLabel, appUser.firstName),
          _getDataCard(context, S.of(context).signUpPageLastNameFieldLabel, appUser.lastName),
          _getDataCard(context, S.of(context).signUpPageEmailFieldLabel, appUser.email),
          Padding(
            padding: const EdgeInsets.only(left: 8.0, top: 8),
            child: TextButton(onPressed: () => _onTapSignOut(context), child: Text(
      ...
      child: Card(
        child: Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(label, style: Theme.of(context).textTheme.headline3.copyWith(),),
              SizedBox(height: 8,),
              Text(value, style: Theme.of(context).textTheme.bodyText1.copyWith(),)
            ],
          ),
        ),
      ),
  ...
  _onTapSignIn(BuildContext context){
    ExtendedNavigator.of(context).pushSignInPage().then((appUser) => {
      _onTapSuccessCallbackButton(context, appUser)
    });
  }

  _onTapSuccessCallbackButton(BuildContext context, AppUser appUser){
    print('YD --> onTap Sign In Success Inside');
    if(appUser != null){
      print('YD --> onTap Sign In Success Inside');
  ...
  _onTapSignOut(BuildContext context){
    ...
    bottomSheetMessageHelper.showMessage(type: WidgetType.WARNING, message : S.of(context).signoutSuccessMessage,
        onTapSubmitCallback: () => _onTapSignOutCallbackButton(context), context: context);
  }
