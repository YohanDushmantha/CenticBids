...
class BiddingBloc extends Bloc<BiddingEvent, BiddingState> with ErrorHandlingBlocMixin{
  int biddingAmount = 0;
  int latestBid = 0;
  final BidNowUsecase bidNowUsecase;
  BiddingBloc({this.bidNowUsecase}) : super(BiddingInitialState(biddingAmount: 0));
  ...
  Stream<BiddingState> mapEventToState(
    ...
    if(event is SetBiddingAmount){
      ...
      if(biddingAmount > latestBid){
        ...
        yield BiddingError(biddingAmount: biddingAmount, runtimeError : RuntimeError(message: 'Bid amount should be greater than to latest bid.'));
      }
    } else if (event is BidNowEvent){
      yield LoadingBidNow(biddingAmount);
      final failureOrCreated = await bidNowUsecase(Params(uid: event.appUser?.uid, biddingAmount: biddingAmount,
      auctionId: event?.auction?.id, auctionItem: event?.auction));
  ...
  Stream<BiddingState> _eitherCreatedBidOrError(
    ...
    yield either.fold(
            (failure) => BiddingError(biddingAmount: biddingAmount, runtimeError: mapFailureToRuntimeError(failure)),
            (bid) =>
            LoadedBidNow(bid: bid));
  }
}
